...
public class GroupService : IGroupService
{
    ...
    public GroupService(ApplicationDbContext context, ILessonService lessonService)//,INotificationService notificationService)//, ILessonService lessonService, IUserService userService, INotificationService notificationService)
    {
        _context = context;
    ...
    #region Get
    public IQueryable<Group> GetItems() => _context.Groups
            .Include(x => x.Lessons)
            .Include(x => x.GroupStudents).ThenInclude(x => x.Student).ThenInclude(x => x.ApplicationUser).ThenInclude(x => x.ConnectedUsersInfo).ThenInclude(x => x.BaseUserInfo)
            .Include(x => x.GroupTeachers).ThenInclude(x => x.Teacher).ThenInclude(x => x.ApplicationUser).ThenInclude(x => x.ConnectedUsersInfo).ThenInclude(x => x.BaseUserInfo)
            .Include(x => x.DaySchedules)
    ...
    public async Task<Group> GetAsync(Guid id)
    {
       ...
       //await base.GetBaseAsync(id);
        
        var groups = await GetItems().ToListAsync();
    ...
    public async Task<Group> CreateAsync(Group item, Guid[] studentId, string[] dayName, TimeSpan[] startTime)
    {
        ...
        var groupDb = new Group();//await base.BaseCreateAsync(item);
    ...
    public async Task<Group> UpdateAsync(Group item, Guid[] studentId, string[] dayName, TimeSpan[] startTime)
    {
        ...
        await Validate(item, studentId, dayName, startTime);
        group = new Group();//await base.BaseUpdateAsync(item);
    ...
    public async Task Validate(Group item, Guid[] studentId, string[] dayName, TimeSpan[] startTime)
    {
        ...
        var teacher = await _context.Teachers.FirstOrDefaultAsync(x => x.Id == item.TeacherId);
        var students = _context.Students.Where(s => studentId.Contains(s.Id)).Where(x => !x.IsDeleted).Include(x => x.ApplicationUser).ThenInclude(x => x.ConnectedUsersInfo).ThenInclude(x => x.BaseUserInfo).ToList();
