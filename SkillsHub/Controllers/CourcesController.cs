...
public class CourcesController : Controller
{
    ...
    public IActionResult Index()
    {
        ...
        var courcesNames = _context.Courses.AsQueryable();
	...
	[HttpPost]
	public async Task SaveCourcesNames(Course item)
	{
        if (item.Id == Guid.Empty) return;
	...
	[HttpPost]
	public async Task<IActionResult> CreateCourceName(Course  item)
	{
        try
        {
    ...
    public IActionResult DeleteCourceName(Course item)
    {
        var courceName = _context.Courses.FirstOrDefault(c => c.Id == item.Id);
        if (courceName == null) { return RedirectToAction("Index"); }
        //_context.CourseNames.Remove(courceName);
        courceName.IsDeleted = true;
        _context.Courses.Update(courceName);
